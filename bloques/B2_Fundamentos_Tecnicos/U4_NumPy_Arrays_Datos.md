...
# Simular una imagen en escala de grises (valores 0-255)
...
def filtro_promedio(img):
    ...
    for i in range(1, img.shape[0]-1):
        for j in range(1, img.shape[1]-1):
            resultado[i, j] = np.mean(img[i-1:i+2, j-1:j+2])
    return resultado

imagen_filtrada = filtro_promedio(imagen_normalizada)

print(f"Imagen original: {imagen.shape}, rango: [{imagen.min()}, {imagen.max()}]")
print(f"Normalizada: rango: [{imagen_normalizada.min():.2f}, {imagen_normalizada.max():.2f}]")
```

### Ejemplo 2: Operaciones para machine learning

```python
# Simulación de dataset
np.random.seed(42)
n_muestras, n_caracteristicas = 1000, 5
X = np.random.normal(0, 1, (n_muestras, n_caracteristicas))
y = np.random.randint(0, 2, n_muestras)

# Normalización Z-score
X_media = np.mean(X, axis=0)
X_std = np.std(X, axis=0)
X_normalizado = (X - X_media) / X_std

# División train/test (80/20)
indices = np.random.permutation(n_muestras)
split_idx = int(0.8 * n_muestras)
train_idx, test_idx = indices[:split_idx], indices[split_idx:]

X_train, X_test = X_normalizado[train_idx], X_normalizado[test_idx]
y_train, y_test = y[train_idx], y[test_idx]

print(f"Datos de entrenamiento: {X_train.shape}")
print(f"Datos de prueba: {X_test.shape}")
print(f"Media de X_train: {np.mean(X_train, axis=0)}")  # Debe ser ~0
print(f"Std de X_train: {np.std(X_train, axis=0)}")     # Debe ser ~1
```

### Ejemplo 3: Cálculo de distancias (útil para clustering/kNN)
...
def matriz_distancias(puntos):
    ...
    for i in range(n):
        for j in range(n):
            matriz[i, j] = distancia_euclidiana(puntos[i], puntos[j])
    return matriz

# Vectorización eficiente usando broadcasting
def matriz_distancias_vectorizada(puntos):
    """Versión vectorizada mucho más rápida."""
    # puntos: (n_puntos, n_dimensiones)
...
# Ejemplo de uso
puntos = np.random.random((100, 3))  # 100 puntos en 3D

# Comparar velocidad
import time

start = time.time()
dist_lenta = matriz_distancias(puntos)
tiempo_lenta = time.time() - start

start = time.time()
dist_rapida = matriz_distancias_vectorizada(puntos)
tiempo_rapida = time.time() - start

print(f"Versión bucles: {tiempo_lenta:.4f}s")
print(f"Versión vectorizada: {tiempo_rapida:.4f}s")
print(f"Aceleración: {tiempo_lenta/tiempo_rapida:.1f}x")
print(f"Matrices iguales: {np.allclose(dist_lenta, dist_rapida)}")
```
---
...
# Datos de ventas: 4 productos, 3 meses
# Filas = productos, Columnas = meses (Enero, Febrero, Marzo)
ventas = np.array([
    [120, 135, 150],  # Producto A
    [80,  95,  110],  # Producto B
    [200, 180, 210],  # Producto C
    [50,  60,  55]    # Producto D
])
...
# 1. Ventas totales por producto (suma por fila)
ventas_por_producto = np.sum(ventas, axis=1)
print(f"\nVentas totales por producto: {ventas_por_producto}")
# [405 285 590 165]

# 2. Ventas totales por mes (suma por columna)
ventas_por_mes = np.sum(ventas, axis=0)
print(f"Ventas totales por mes: {ventas_por_mes}")
# [450 470 525]

# 3. Producto con mayores ventas
idx_mejor = np.argmax(ventas_por_producto)
print(f"Mejor producto: Producto {'ABCD'[idx_mejor]} con {ventas_por_producto[idx_mejor]} ventas")

# 4. Crecimiento de ventas (% entre marzo y enero)
crecimiento = ((ventas[:, 2] - ventas[:, 0]) / ventas[:, 0]) * 100
print(f"\nCrecimiento por producto (%): {np.round(crecimiento, 2)}")
# [25.   37.5   5.   10. ]

# 5. Productos que superan el promedio general
promedio_general = np.mean(ventas)
productos_sobre_promedio = ventas > promedio_general
print(f"\nPromedio general de ventas: {promedio_general:.2f}")
print("Productos sobre el promedio:")
print(productos_sobre_promedio)

# 6. Aplicar descuento del 10% a productos con ventas < 100
ventas_con_descuento = ventas.copy()
mascara_bajo_rendimiento = ventas < 100
ventas_con_descuento[mascara_bajo_rendimiento] *= 0.9
print(f"\nVentas ajustadas con descuento:\n{ventas_con_descuento}")
```
...
# Cárea un array con los números del 1 al 10
...
## Tarea para casa (OPCIONAL pero recomendada)

Para reforzar lo aprendido:

1. **Instala el entorno** en tu ordenador personal si no lo has hecho
2. **Practica los ejercicios** de esta sesión hasta que te salgan sin mirar
3. **Explora NumPy**: 
   - Prueba funciones que no vimos: `np.concatenate`, `np.stack`, `np.split`
   - Investiga sobre `np.random`: genera datos aleatorios de diferentes distribuciones
4. **Desafío**: Crea un script que:
   - Genere una "imagen" aleatoria de 10x10
   - Aplique 3 transformaciones diferentes (blur, contraste, rotación simulada)
   - Muestre el resultado con `print()` (prximamente veremos visualización real!)
